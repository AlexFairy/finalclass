...
def update_customer(id):
    try:
        ...
        if customer.email != update_data["email"] or customer.password != update_data["password"]:
            return jsonify({"message": "Invalid email or password"}), 401

        customer.name = update_data["name"]
        customer.phone_number = update_data["phone_number"]

        cache.delete(f"/customers/{id}")

        db.session.commit()

        return jsonify({"id": customer.id, "name": customer.name, "phone_number": customer.phone_number}), 200
    except Exception:
